...
def get_logits():
    ...
    texts, labels = dataloader.read_pickle(args.dataset_path)
    ...
    for idx, (text, true_label) in enumerate(data):
        logits = model.text_pred_logit([text], batch_size=args.batch_size)
        print(np.argmax(logits[0]), true_label)
        if np.argmax(logits[0]) == true_label:
            acc += 1
        tot += 1
