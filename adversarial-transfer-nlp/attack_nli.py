...
class NLI_infer_InfoBERT(nn.Module):
    def __init__(self,
                 pretrained_dir,
                 max_seq_length=128,
                 batch_size=32):
        super(NLI_infer_InfoBERT, self).__init__()
    ...
    def text_pred(self, text_data):
        ...
        for input_ids, input_mask, segment_ids in dataloader:
            ...
            with torch.no_grad():
                if 'roberta' in self.pretrained_dir:
                    logits = self.model(input_ids=input_ids, attention_mask=input_mask)
                else:
...
def main():
    ...
    parser.add_argument("--target_model",
                        type=str,
                        required=True,
                        choices=['infersent', 'esim', 'bert', 'infobert', 'roberta'],
    ...
    if args.target_model == 'esim':
        ...
        model = NLI_infer_InferSent(args.target_model_path,
                                    ...
                                    data=data,
                                    batch_size=args.batch_size)
    elif args.target_model == 'infobert' or args.target_model == 'roberta':
        model = NLI_infer_InfoBERT(args.target_model_path)
    else:
    ...
    if args.target_model == 'bert':
        labeldict = {0: "contradiction",
                     1: "entailment",
                     2:  "neutral"}
    elif args.target_model == 'infobert':
        labeldict = {0: "contradiction",
                     1: "entailment",
                     2:  "neutral"}
    ...
    with open(os.path.join(args.output_dir,  f'{args.target_model}_adv_dataset.jsonl'), 'w') as ofile:
        for orig_premise, orig_hypothesis, adv_hypothesis, \
            true_label, new_label in zip(orig_premises, orig_hypotheses, adv_hypotheses,
                                        true_labels, new_labels):
            data = {"orig_preimise": orig_premise, "orig_hypothesis": orig_hypothesis, "adv_hypothesis": adv_hypothesis,
                    "true_label": labeldict[true_label], "new_label": labeldict[new_label]}
            import json
