...
class AGProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        ...
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "train_tok.csv"), "\""), "train")
    ...
    def get_dev_examples(self, data_dir):
        ...
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "test_tok.csv"), "\""), "dev")
...
def main():
    ...
    # load from source models
    if 'ag' in args.bert_model:
        source_labels = 4
    else:
        source_labels = 2
    model = BertForSequenceClassification.from_pretrained(args.bert_model,
                                                          cache_dir=cache_dir,
                                                          num_labels=source_labels)
