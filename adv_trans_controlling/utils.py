...
# Models                          #
###################################
def get_models(args, train=True, as_ensemble=False, model_file=None, leaky_relu=False, dataset=None):
...
# data loader                     #
###################################
def get_loaders(args, add_gaussian=False, dataset=None):
    ...
    testloader = DataLoader(testset, num_workers=4, batch_size=128, shuffle=False, pin_memory=True)
...
class TransferLoader:
    def __init__(self, source, seed, target):
    ...
    def __next__(self):
        try:
            x, y = next(self.source)
            si, sl = next(self.seed)
            ti, tl = next(self.target)
            return x, y, si, sl, ti, tl
        except StopIteration as e:
            raise StopIteration
...
def get_schedulers(args, optimizers):
    scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer=optimizers, T_max=200)
    return scheduler
