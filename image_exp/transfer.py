...
parser.add_argument("--source_classes", default = 5, type = int)
...
if args.data == 'cifar10':
    ...
    trainset = torchvision.datasets.CIFAR100('../data/cifar100', train = False, transform=transform_test, download = True)
    trainloader = torch.utils.data.DataLoader(trainset,
                                              batch_size=args.batch_size,
                                              shuffle=True,
                                              num_workers=args.workers)
...
#     testset = CIFAR100_SubLoader('../data/cifar100', superclass = args.superclass, train = False, transform=transform_test,download=True)
    testset = torchvision.datasets.CIFAR100('../data/cifar100', train = False, transform=transform_test, download = True)
    testloader = torch.utils.data.DataLoader(testset,
                                             batch_size=100,
                                             shuffle=True,
                                             num_workers=args.workers)
...
model = models.__dict__[args.arch](num_classes = args.source_classes)
state_dict = torch.load('checkpoint/'+args.load_name)['model']
new_state_dict = {}
for k,v in state_dict.items():
    new_state_dict[k[7:]] = v
model.load_state_dict(new_state_dict)
if args.arch == 'alexnet':
    model.classifier = nn.Linear(model.classifier.weight.shape[1], num_classes)
elif args.arch == 'fcnet':
    model.fc5 = nn.Linear(model.fc5.weight.shape[1], num_classes)
elif args.arch == 'le_net':
    model.fc3 = nn.Linear(model.fc3.weight.shape[1], num_classes)
elif args.arch == 'vit':
    model.transformer.dense_final = nn.Linear(model.transformer.dense_final.weight.shape[1], num_classes)
else:
    model.linear = nn.Linear(model.linear.weight.shape[1], num_classes)
if args.cuda:
...
if args.transfer_last:
    if args.arch == 'alexnet':
        opt_param = [v for k,v in model.named_parameters() if 'module.classifier' in k]
    elif args.arch == 'fcnet':
        opt_param = [v for k,v in model.named_parameters() if 'fc5' in k]
    elif args.arch == 'le_net':
        opt_param = [v for k,v in model.named_parameters() if 'fc3' in k]
    elif args.arch == 'vit':
        opt_param = [v for k,v in model.named_parameters() if 'transformer.dense_final' in k]
    else:
...
if args.optim=='sgd':
    ...
    optimizer = torch.optim.Adam(opt_param,
                               lr=args.lr,
                               weight_decay=args.weight_decay)
...
def validate(epoch):
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        if args.transfer_last: 
            torch.save(state, './checkpoint/'+'transfer_last_'+args.load_name)
        else:
            torch.save(state, './checkpoint/'+'transfer_' + args.load_name)
        best_acc = acc
