...
ref_state_dict = torch.load('checkpoint/' + 'target_model.pth')['model']
new_ref_state_dict = {}
...
for batch_idx,(images,labels) in enumerate(tqdm(testloader)):
    ...
    true_outputs = ref_model(images)  
    if true_outputs.max(1, keepdim = True)[1].item()!= labels[0].item():
        continue
    losses.append(criterion(inference_model(images), labels).item())
    
    adversarials = attack_method(transfer_model, images) 
    outputs = ref_model(adversarials)
    
    base_adversarials = attack_method(ref_model, images)
    base_outputs = ref_model(base_adversarials)
    ...
    v1 = (transfer_model(images) - transfer_model(adversarials)).flatten().data.cpu().numpy()
    v1 = v1/np.linalg.norm(v1)
    v2 = (true_outputs - outputs).data.cpu().numpy()
